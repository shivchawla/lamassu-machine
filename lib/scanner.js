...
// const manatee = require('manatee') -- needs license
...
function scan (mode, resultCallback, captureCallback) {
  ...
  function capture () {
    ...
    cam.capture(function (success) {
      ...
      try {
        greyscale = jpg.decompressSync(frame, { format: jpg.FORMAT_RGBA })
      } catch (err) {
...
console.log(typeof(greyscale));
console.log(greyscale);
	//const imageData = Uint8ClampedArray.from(greyscale.buffer);
const imageData =  new Uint8ClampedArray(greyscale.buffer);
console.log(typeof(imageData));
console.log(imageData);
	var result = jsQR(imageData, width, height);
console.log("Result");
console.log(result);

    if (!result) return _callback()
    _callback(null, result.data.toString())
  })
}

function scanPDF417 (callback) {
  scan('photoId', callback, function (modeConfig, width, height, frame, greyscale, _callback) {
    var result = null; //manatee.scanPDF417(greyscale, width, height)

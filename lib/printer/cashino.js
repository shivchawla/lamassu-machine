...
function printTest(printerCfg = { address: '/dev/ttyS1' }) { 
  return new Promise((resolve, reject) => {
    const device  = new escpos.SerialPort(printerCfg.address);
...
async function printReceipt(data, printerCfg = { address: '/dev/ttyS1' }) { 
  return new Promise((resolve, reject) => {
    ...
    device.open(function(){
      ...
      // QRcode
      await printer.qrcode(data.address)
...
async function printWallet (wallet, printerCfg) {
  return new Promise((resolve, reject) => {
    ...
    device.open(function(){
      ...
      await printer.text('BTC PAPER WALLET');
      await printer.nextLine();
      ...
      // QR Code
      await printer.qrcode(wallet.privateKey);
